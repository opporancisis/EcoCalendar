...
create table user (
  ...
  constraint uq_user_1 unique (NICK,EMAIL),
...
alter table blog_post add constraint fk_blog_post_owner_1 foreign key (owner_id) references user (id) on delete restrict on update restrict;
create index ix_blog_post_owner_1 on blog_post (owner_id);
...
alter table event add constraint fk_event_author_3 foreign key (author_id) references user (id) on delete restrict on update restrict;
create index ix_event_author_3 on event (author_id);
alter table event add constraint fk_event_parent_4 foreign key (parent_id) references grand_event (id) on delete restrict on update restrict;
create index ix_event_parent_4 on event (parent_id);
alter table event add constraint fk_event_city_5 foreign key (city_id) references city (id) on delete restrict on update restrict;
create index ix_event_city_5 on event (city_id);
alter table geo_coords add constraint fk_geo_coords_event_6 foreign key (event_id) references event (id) on delete restrict on update restrict;
create index ix_geo_coords_event_6 on geo_coords (event_id);
alter table grand_event add constraint fk_grand_event_author_7 foreign key (author_id) references user (id) on delete restrict on update restrict;
create index ix_grand_event_author_7 on grand_event (author_id);
alter table karma_change add constraint fk_karma_change_user_8 foreign key (user_id) references user (id) on delete restrict on update restrict;
create index ix_karma_change_user_8 on karma_change (user_id);
alter table linked_account add constraint fk_linked_account_user_9 foreign key (user_id) references user (id) on delete restrict on update restrict;
create index ix_linked_account_user_9 on linked_account (user_id);
alter table message add constraint fk_message_owner_10 foreign key (owner_id) references user (id) on delete restrict on update restrict;
create index ix_message_owner_10 on message (owner_id);
alter table token_action add constraint fk_token_action_targetUser_11 foreign key (target_user_id) references user (id) on delete restrict on update restrict;
create index ix_token_action_targetUser_11 on token_action (target_user_id);
alter table user add constraint fk_user_city_12 foreign key (city_id) references city (id) on delete restrict on update restrict;
create index ix_user_city_12 on user (city_id);
alter table user add constraint fk_user_countryForUnknownCity_13 foreign key (country_for_unknown_city_id) references country (id) on delete restrict on update restrict;
create index ix_user_countryForUnknownCity_13 on user (country_for_unknown_city_id);



alter table blog_post_uploaded_file add constraint fk_blog_post_uploaded_file_bl_01 foreign key (blog_post_id) references blog_post (id) on delete restrict on update restrict;

alter table blog_post_uploaded_file add constraint fk_blog_post_uploaded_file_up_02 foreign key (uploaded_file_id) references uploaded_file (id) on delete restrict on update restrict;
