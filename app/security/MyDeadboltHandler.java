...
public class MyDeadboltHandler extends AbstractDeadboltHandler {
	...
	public F.Promise<Result> beforeAuthCheck(final Http.Context context) {
		if (PlayAuthenticate.isLoggedIn(context.session())) {
			...
			context.flash().put(
					Application.FLASH_ERROR_KEY,
					Messages.get("error.you.need.to.log.in.first.to.view",
							originalUrl));
			return F.Promise.promise(() -> redirect(PlayAuthenticate
					.getResolver().login()));
		}
	}
	...
	public DynamicResourceHandler getDynamicResourceHandler(
		...
		return DYNAMIC_RESOURCE_HANDLER;
	}
